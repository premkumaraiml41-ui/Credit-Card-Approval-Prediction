...
def _read_remote_or_local(remote_urls, local_path):
    """Try a list of remote URLs (in order) and fall back to a local file.

    remote_urls: list of str
    local_path: str
    ...
    # final fallback: local path
    if local_path and os.path.exists(local_path):
        return pd.read_csv(local_path)

    raise FileNotFoundError(
        f"None of the remote URLs worked and local file not found: {local_path}"
    )


train_urls = [
    "https://raw.githubusercontent.com/premkumar-ul/Credit-card-approval-prediction-classification/refs/heads/main/dataset/train.csv",
...
test_urls = [
    "https://raw.githubusercontent.com/premkumar-ul/Credit-card-approval-prediction-classification/refs/heads/main/dataset/test.csv",
    "https://raw.githubusercontent.com/semasuka/Credit-card-approval-prediction-classification/refs/heads/main/dataset/test.csv",
]

train_original = _read_remote_or_local(train_urls, os.path.join("dataset", "train.csv"))
test_original = _read_remote_or_local(test_urls, os.path.join("dataset", "test.csv"))
...
def make_prediction():
    ...
    local_candidates = [
        os.path.join("models", key),
        key,
        os.path.join(os.getcwd(), "models", key),
    ]
    ...
    for local_path in local_candidates:
        try:
            ...
            logger.error(f"Failed to load local model {local_path}: {e}")

    st.error("Local model file not found. Please place gradient_boosting_model.sav under the models/ directory.")
    return None
